...
同时处理安卓微信等外部悬浮小窗输入时的视口联动：当酒馆窗口已经失去焦点、外部输入法又压缩了酒馆可视区域时，插件会锁定酒馆在键盘出现前的布局高度，使底部输入栏继续停留在原位置并被键盘遮挡，而不是重新浮到键盘上方；返回酒馆或收起外部键盘后会自动解除锁定。
...
- 微信等外部小窗关闭、键盘收起或用户返回酒馆后，布局锁定会立即解除。
